...
LveDevice::LveDevice(LveWindow &window) : window{window} {
  ...
  setupDebugMessenger(); // sets better errors on build versions
  createSurface(); // connects window we created and Vulkan
  pickPhysicalDevice(); // picks GPU (this only uses 1 GPU. It is possible to handle more but is alot more complex to implement properly.).
  createLogicalDevice(); // describes what features of physical device to use
  createCommandPool(); // command buffer allocations become easier
}
