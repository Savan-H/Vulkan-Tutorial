...
namespace lve {
	...
	PipelineConfigInfo LvePipeline::defaultPipelineConfigInfo(uint32_t width, uint32_t height)
	{
		...
		configInfo.viewport.minDepth = 0.0f; // z value, depth value.
		configInfo.viewport.maxDepth = 1.0f;

		configInfo.scissor.offset = { 0, 0 };
		configInfo.scissor.extent = { width, height }; // How much of the screen to "cut".

		configInfo.rasterizationInfo.sType = VK_STRUCTURE_TYPE_PIPELINE_RASTERIZATION_STATE_CREATE_INFO;
		configInfo.rasterizationInfo.depthClampEnable = VK_FALSE; // Don't clamp depth since < 0 means behind, > 1 means too far ahead.
		...
		// How rasterizer handles the edges of triangles. MSAA, multisample anti-aliasing, how much of the fragment is in the triangle and shading accordingly.
	...
	void LvePipeline::createGraphicsPipeline(const std::string& vertFilePath, const std::string& fragFilepath, const PipelineConfigInfo& configInfo)
	{
		...
		shaderStages[0].stage = VK_SHADER_STAGE_VERTEX_BIT; // VERT SHADER
		...
		shaderStages[1].stage = VK_SHADER_STAGE_FRAGMENT_BIT; // FRAG SHADER
		shaderStages[1].module = fragShaderModule;
		shaderStages[1].pName = "main"; // entry function in FRAG shader
		...
		// Can be faster for GPU to derive a new graphics pipeline from an old one.
		...
		if (vkCreateGraphicsPipelines(lveDevice.device(), VK_NULL_HANDLE, 1, &pipelineInfo, nullptr, &graphicsPipeline) != VK_SUCCESS) {
			throw std::runtime_error("Failes to create graphics pipeline.");
		}
	}
